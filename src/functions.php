...
function _if($cond, $onTrue = true, $onFalse = false) {return \EasyCallback\f($cond)->ecIf($onTrue, $onFalse);}
function _or($p1, $p2, $p3 = false, $p4 = false) {return \EasyCallback\f($p1)->ecOr($p2, $p3, $p4);}
function eq($p1) {return \EasyCallback\f()->ecEq($p1);}
function _and($p1, $p2, $p3 = true, $p4 = true) {return \EasyCallback\f($p1)->ecAnd($p2, $p3, $p4);}
function concat($string1, $string2) {
    $args = func_get_args();
    array_shift($args);
    return new \EasyCallback\Func\Concat(\EasyCallback\f($string1), $args);
}
